...
export default class FileToolbar extends Component<FileToolbarProps, FileToolbarState>{
    ...
    componentDidMount() {
        ...
        this.pasteListen = async function (event:any) { // 监听粘贴里的文件
                ...
                if (files.length === 1) {
                    self.showFile(files[0]);
                        ...
                        for (let i = 0; i < files.length; i++) {
                            let file = files[i];
                            const res = await self.getFilePromise(file)
                            more_files.push(res)
                        }
                        for (let i = 0; i < more_files.length; i++) {
                            const {  file, previewUrl,width,height,fileType } = more_files[i]
                            if(fileType === "image") {
                                await conversationContext.sendMessage(new ImageContent(file,previewUrl,100,100))
                            }else{
                                await conversationContext.sendMessage(new FileContent(file))
                            }
                        }
                }
            }
        ...
        conversationContext.setDragFileCallback(async (files: any)=>{
            if (files.length === 1) {
                self.showFile(files[0]);
            } else {
                // 处理多个文件并直接发送
    ...
    async onFileChange() {
        ...
        if ( this.$fileInput.files.length === 1) {
            ...
            const {conversationContext} = this.props
            for (let i = 0; i < files.length; i++) {
                const {  file, previewUrl,width,height,fileType } = files[i]
                if(fileType === "image") {
                    await conversationContext.sendMessage(new ImageContent(file,previewUrl,100,100))
                }else{
                    await conversationContext.sendMessage(new FileContent(file))
        ...
        return <div className="wk-filetoolbar" >
            ...
            this.chooseFile()
                ...
                <div className="wk-filetoolbar-content-icon">
                    <img src={icon}></img>
                    <input onClick={this.onFileClick} onChange={this.onFileChange.bind(this)} ref={(ref) => { this.$fileInput = ref }} type="file" multiple={true} accept="*" style={{ display: 'none' }} />
                </div>
            </div>
            {
                showDialog ? (
