...
const createSubWindow = async (data) => {
  const _data = JSON.parse(data)
  if (subWindow[_data.key]) {
    subWindow[_data.key].show(); // 显示窗口
    subWindow[_data.key].focus();
    return
  }
  const NODE_ENV = process.env.NODE_ENV;
  const { width, height } = screen.getPrimaryDisplay().workAreaSize;
  subWindow[_data.key] = new BrowserWindow({
    width: 960,
    height: 600,
    minWidth: 960,
    minHeight: 600,
  ...
  subWindow[_data.key].center();
  subWindow[_data.key].once("ready-to-show", () => {
    subWindow[_data.key].show(); // 显示窗口
    subWindow[_data.key].focus();
    // ipcMain.emit("mytest")
    subWindow[_data.key].webContents.send('newWindowChatData', data)
  });

  subWindow[_data.key].on("close", (e: any) => {
    if (forceQuit || !tray) {
      subWindow[_data.key] = null;
    } else {
      e.preventDefault();
      if (subWindow[_data.key].isFullScreen()) {
        subWindow[_data.key].setFullScreen(false);
        subWindow[_data.key].once("leave-full-screen", () => subWindow[_data.key].hide());
      } else {
        subWindow[_data.key].hide();
      }
    }
  });
  if (NODE_ENV === "development") subWindow[_data.key].loadURL("http://localhost:3000");
  if (NODE_ENV !== "development") {
    process.env.DIST_ELECTRON = join(__dirname, "../");
    const WEB_URL = join(process.env.DIST_ELECTRON, "../build/index.html");
    subWindow[_data.key].loadFile(WEB_URL);
  }
  ...
  subWindow[_data.key].setMenu(null);
};
...
app.whenReady()
  .then(res => {
    ipcMain.on("showConversationNewWindow", (e, data) => {
      // 打開New Window
